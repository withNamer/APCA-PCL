...
from networks.projector import projectors, classifier, projectors_local, PatchEmbed, projectors_feature,projectors_linear,projectors_linear_before,projectors_unet,projectors_linear_unet,projectors_linear_p,projectors_linear_p2
...
def net_factory(net_type="unet", in_chns=1, class_num=3):
    if net_type == "unet":
        ...
        net = Effi_UNet('efficientnet-b3', encoder_weights='imagenet', in_channels=in_chns, classes=class_num).cuda()
        ...
        net = projectors().cuda()
    elif net_type == "projector_local":
        net = projectors_local().cuda()
    elif net_type == "PatchEmbed":
        net = PatchEmbed().cuda()
    elif net_type == "projectors_feature":
        net = projectors_feature().cuda()
    elif net_type == "projectors_linear":
        net = projectors_linear().cuda()
    elif net_type == "projectors_linear_before":
        net = projectors_linear_before().cuda()
    elif net_type == "projectors_unet":
        net = projectors_unet().cuda()
    elif net_type == "projectors_linear_unet":
        net = projectors_linear_unet().cuda()  
    elif net_type == "projectors_linear_p":
        net = projectors_linear_p().cuda()
    elif net_type == "projectors_linear_p2":
        net = projectors_linear_p2().cuda()
    else:
        net = None
    return net
