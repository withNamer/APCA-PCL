...
import math
from augmentations.adv_transformation_base import AdvTransformBase  # noqa
...
def get_base_grid(batch_size, image_height, image_width, image_depth=None, device=torch.device('cuda')):
    ...
    if image_depth is None:
        y_ind, x_ind = torch.meshgrid(
            [torch.linspace(-1, 1, image_height,device=device), torch.linspace(-1, 1, image_width,device=device)])  # image space [0-H]；,indexing='ij'
