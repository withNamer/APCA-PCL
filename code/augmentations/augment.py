...
from augmentations.adv_morph import *
...
def transform(image, label, logits=None, crop_size=(224, 224), scale_size=(0.8, 1.0), augmentation=True):
...
def randomGeneratorWithLogits(image, label, logit, output_size=[224, 224]):
    new_data = []
    new_target = []
    new_logits = []
    _, _, x, y = image.shape
    ...
    for i in range(image.shape[0]):
        ...
        # 一个缩放函数
        image_i = zoom(image_i, (output_size[0] / x, output_size[1] / y), order=0)
        label_i = zoom(label_i, (output_size[0] / x, output_size[1] / y), order=0)
        logit_i = zoom(logit_i, (output_size[0] / x, output_size[1] / y), order=0)
