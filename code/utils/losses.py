...
#         self.temperature = temperature
#         self.base_temperature = base_temperature
#         self.nce_includes_all_negatives_from_minibatch = True
#         self.cross_entropy_loss = torch.nn.CrossEntropyLoss()
# #         self.cross_entropy_loss = torch.nn.CrossEntropyLoss(reduction = 'none')
#         self.mask_dtype = torch.bool
# #         self.mask_dtype = torch.uint8 if version.parse(torch.__version__) < version.parse('1.2.0') else torch.bool
...
class contrastive_loss_sup(torch.nn.Module):
    ...
    def forward(self, feat_q, feat_k):
        ...
        diagonal = torch.eye(npatches, device=feat_q.device, dtype=self.mask_dtype)[None, :, :] # (1, 3136, 3136)
...
class contrastive_loss_sup_seq(torch.nn.Module):
    def __init__(self, temperature=0.07, base_temperature=0.07):
        """
        Contrastive Learning for Unpaired Image-to-Image Translation
        models/patchnce.py
        """
        super(contrastive_loss_sup_seq, self).__init__()
        self.temperature = temperature
        self.base_temperature = base_temperature
        self.nce_includes_all_negatives_from_minibatch = False
        self.cross_entropy_loss = torch.nn.CrossEntropyLoss()
...
#for unlabel data
    def __init__(self, temperature=0.07, base_temperature=0.07, use_queue = True, max_queue = 1):
        """
        Contrastive Learning for Unpaired Image-to-Image Translation
        models/patchnce.py
        """
...
class NegEntropy(object):
    def __call__(self, outputs_list):
        ...
        current_prob = torch.softmax(outputs_list, dim=1)
        entropy_results += torch.sum(current_prob.log() * current_prob, dim=1).mean()
...
def label_onehot(inputs, num_segments):
    ...
    return outputs.scatter_(1, inputs.unsqueeze(1), 1.0).cuda()

def semi_crc_loss(inputs, pseudo_outputs, targets, threshold=0.6, neg_threshold=0.3, conf_mask=True):
    if not conf_mask:
    ...
    # for positive
    pos_weight = torch.gather(targets_prob, 1, pseudo_outputs.unsqueeze(1)).squeeze(1)
    ...
    if not torch.any(pos_mask):
        ...
        positive_loss_mat_ = positive_loss_mat[mask]
    ...
    return positive_loss_mat_.mean() + positive_loss_mat.mean() + negative_loss_mat.mean(), None # 

def semi_mycrc_loss(inputs, pseudo_outputs, targets, threshold=0.6, neg_threshold=0.3, conf_mask=True):
    if not conf_mask:
        raise NotImplementedError
    mask = ~(pseudo_outputs == -1)
    pseudo_outputs.masked_fill_(~mask, 0) 
    ...
    if not torch.any(pos_mask):
        positive_loss_mat = torch.tensor([.0], device=targets.device)
        # positive_loss_mat_ = torch.tensor([.0], device=targets.device)
    else:
        positive_loss_mat = F.nll_loss(torch.log_softmax(inputs, dim=1), pseudo_outputs, reduction="none")
        positive_loss_mat = positive_loss_mat * pos_weight
        positive_loss_mat_ = positive_loss_mat[mask]
        positive_loss_mat = positive_loss_mat[pos_mask]
...
class ConsistencyWeight(object):
    ...
    def __init__(self):
        self.final_w = 1.0
        self.iter_per_epoch = 11
        self.start_iter = 0 * 11
        self.rampup_length = 40 * 11
    ...
    def __call__(self, current_idx):
        ...
        self.current_rampup = self.rampup_func(current_idx-self.start_iter, self.rampup_length)
