...
def load_checkpoint_more(path,  model1, model1_t, optimizer1, optimizer1_t, optimizer3, projector_1, projector_3, cta,):  # 在load上需要明确具体对象
    ...
    model1_t.load_state_dict(checkpoint["model1_t_state_dict"])
    # transfer optimizer state_dict from checkpoint to model
    optimizer1.load_state_dict(checkpoint["optimizer_state_dict"])
    optimizer1_t.load_state_dict(checkpoint["optimizer1_t_state_dict"])
    optimizer3.load_state_dict(checkpoint["optimizer3_state_dict"])
    projector_1.load_state_dict(checkpoint["projector_1_state_dict"])
    projector_3.load_state_dict(checkpoint["projector_3_state_dict"])
    cta = checkpoint["cta"]
    epoch = checkpoint["epoch"]
    best_performance = checkpoint["best_performance"]
    # track loss
    # loss = checkpoint["loss"]
    return model1, model1_t, optimizer1, optimizer1_t, optimizer3, projector_1, projector_3, cta, epoch, best_performance
        #    loss.item()
...
def restore_model(logger, snapshot_path, model_num=None):
    ...
    try:
        ...
        model, optimizer, start_epoch, performance = load_checkpoint(snapshot_path + "/" + model_checkpoint, model, optimizer)
...
def save_checkpoint_more(epoch, model1, model1_t, optimizer1, optimizer1_t, optimizer3, 
                         projector_1, projector_3, cta, best_performance1, save_mode_path):
    """Saves model as checkpoint"""
    torch.save({"epoch": epoch,
                "model1_state_dict": model1.state_dict(),
                "model1_t_state_dict": model1_t.state_dict(),
                "optimizer1_state_dict": optimizer1.state_dict(),
                "optimizer1_t_state_dict": optimizer1_t.state_dict(),
                "optimizer3_state_dict": optimizer3.state_dict(),
                "projector_1_state_dict": projector_1.state_dict(),
                "projector_3_state_dict": projector_3.state_dict(),
                "cta": cta,
                "best_performance1": best_performance1,}, save_mode_path) # "loss": loss,
    
def save_checkpoint_more_t(epoch, model1, model2, model1_t, 
                         optimizer1, optimizer1_t, optimizer3,
                         projector_1, projector_3, 
                         cta, best_performance1, save_mode_path):
    """Saves model as checkpoint"""
    torch.save({"epoch": epoch,
                "model1_state_dict": model1.state_dict(),
...
class UnifLabelSampler(Sampler):
    ...
    def generate_indexes_epoch(self):
        ...
        for i in range(len(self.images_lists)):
            indexes = np.random.choice(self.images_lists[i], size_per_pseudolabel, replace=(len(self.images_lists[i]) <= size_per_pseudolabel),)
